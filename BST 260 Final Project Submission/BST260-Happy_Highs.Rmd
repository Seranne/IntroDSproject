...
```{r}
whr <- read_csv("https://raw.githubusercontent.com/Seranne/IntroDSproject/master/Source%20Data/World%20Happiness%20Report/2016.csv")
...
Is there a change in the average ranking of a country's happiness and well-being, given the inclusion of a more comprehensive variable list representing its physical, social, cultural, economic and political environments?
...
After selecting the indicators of interest from the shortened WDI dataset, we created individual scores for each category based on the rank that a country achieved for the included variables. 

Composite Energy Score: This score is based on Access to Electricity (% of population), Renewable Energy Consumption (% of final energy consumption), and Fossil fuel energy consumption (% of final energy consumption)
...
#removing rows with NA Variables >  and filling in remaining na's with same country means
 Energy <- Energy[order(-Energy$Year),]
 Energy <- Energy[rowSums(is.na(Energy)) < ncol(Energy)/2, ]
...
Energy_score <- Energy_score %>% select("CountryName", "finalrank_eng")

en_s <- Energy_score
colnames(en_s)[2] <- "Composite_Energy_Rank"

en_s %>%
   arrange(Composite_Energy_Rank) %>%
   slice(1:5)
```

These scores reflect an aggregrate of least fossil fuel energy consumption, and most proportion of renewable energy consumption - the results of which aligned with our expectations given that the top 5 countries are largely smaller in size, and also have relatively smaller populations. 
...
escore <- emissions_score %>% select("CountryName", "finalrank_emission")
em_s <- escore
colnames(em_s)[2] <- "Composite_Emissions_Rank"
 
em_s %>%
   arrange(Composite_Emissions_Rank) %>%
   slice(1:5)
```

This score reflects lowest CO2 emissions in the world. We believe that this data reflects countries which are more agricultural than industrial, and also ones with relatively smaller population sizes. 

Composite Environmental well-being score: This score is based on the proportion of protected areas: both Terrestrial and Marine protected areas (% of total territorial area), and Forest area(% land area)
...
env_s <- environment_score
colnames(env_s)[2] <- "Composite_Environment_Rank"
 
env_s  %>%
   arrange(Composite_Environment_Rank) %>%
   slice(1:5)
```

Communication/Connectivity score: This score is based on Mobile Cellular subscriptions (per 100 people) and Internet Users (per 100 people)
...
Communication <- WDI4 %>% filter(IndicatorCode == "IT.CEL.SETS.P2"|IndicatorCode == "IT.NET.USER.P2")  %>% spread(IndicatorCode, Value)
 ...
 Communication <- Communication[rowSums(is.na(Communication)) < ncol(Communication)/2, ]
...
com_s <-Communication_score
colnames(com_s)[2] <- "Composite_Communications_Rank"
 
com_s %>%
   arrange(Composite_Communications_Rank) %>%
   slice(1:5)
```

Composite Child Welfare score: Based on share of children not in education, employment or training, and Infant Mortality Rate 
...
childwelfare_score <- childwelfare_score %>% select("CountryName","finalscore_child")

chld_s <- childwelfare_score

colnames(chld_s)[2] <- "Composite_ChildWelfare_Rank"
 
chld_s %>%
   arrange(Composite_ChildWelfare_Rank) %>%
   slice(1:5)
```

Food Deficit score: Assessed by the depth of the food deficit (kilocalories per person per day), and returned as ranks with the least depth of food deficit

```{r}
# Load dataset and select indicator
...
deficit_score <- deficit_score %>% select("CountryName", "finalrank_deficit")

def_s <- deficit_score
colnames(def_s)[2] <- "Composite_Deficit_Rank"
 
def_s  %>%
   arrange(Composite_Deficit_Rank) %>%
   slice(1:5)
```

Economic score: This is assessed by assigning positive correlations between a higher rank and higher GDP per capita (in USD), GDP per capita, PPP (in current international dollar), GDP per capita growth (annual %). The ranks are also negatively correlated with Military Expenditure (% of GDP) and Inflation (annual %).
...
#Inflation (annual %) - Country with the lowest annual inflation % will rank 1
gdp_5 <- GDP_base %>% 
...
economy_score <- economy_score %>% select("CountryName", "finalrank_GDP")

es <- economy_score

colnames(es)[2] <- "Composite_Economic_Rank"
es %>%
   arrange(Composite_Economic_Rank) %>%
   slice(1:5)
```

Education score: Assessed as an average aggregate of Youth literacy rate, population 15-24 years, both sexes (%) and Adult literacy rate, population 15+ years, both sexes (%) 

```{r}
# Selecting relevant indicator variables: 
...
Education_score <- Education %>% mutate(finalrank_edu = rank(Education$score_edu)) %>% select("CountryName", "finalrank_edu")

ed_s<-Education_score
colnames(ed_s)[2] <- "Composite_Education_Rank"

ed_s %>%
   arrange(Composite_Education_Rank) %>%
   slice(1:5)

```

Gender score: Assessed by proportion of seats held by women in national parliaments (%)

```{r}
# Select indicator variable to assess gender equity: Proportion of seats held by women in national parliaments (%)
...
Gender_score <- Gender_score %>% select("CountryName", "finalrank_gen")

gs <- Gender_score
colnames(gs)[2] <- "Composite_Gender_Rank"

gs %>%
   arrange(Composite_Gender_Rank) %>%
   slice(1:5)
```

An interesting finding from our ranking, is the fact that Rwanda ranks No. 1 in the list of countries with the highest proportion of women in their parliaments. This is likely a reflection of its traditionally matriarchal society, increasing equity in gender roles in the country. [More](https://www.thedailybeast.com/two-decades-after-genocide-rwandas-women-have-made-the-nation-thrive) on this [here](https://kristof.blogs.nytimes.com/2008/10/07/when-it-comes-to-women-lawmakers-rwanda-leads-the-world/).

Employment score: Assessed by Wage and salaried workers, total (% of total employed)
...
employment_score <- empl %>% mutate(finalrank_empl =
                          as.numeric(as.factor(rank(-(empl$SL.EMP.WORK.ZS))))) %>% select("CountryName", "finalrank_empl")

emp_s <- employment_score
colnames(emp_s)[2] <- "Composite_Employment_Rank"

emp_s %>%
   arrange(Composite_Employment_Rank) %>%
   slice(1:5)

```

Population score: Final rank is positively correlated lower population density, lower GINI indices and  higher life expectancies at birth
...
Population <- WDI4 %>% filter(IndicatorCode == "SI.POV.GINI"|IndicatorCode == "SM.POP.TOTL.ZS"|IndicatorCode == "SP.DYN.LE00.IN" | IndicatorCode == "SP.POP.TOTL"| IndicatorCode == "EN.POP.DNST") %>% spread(IndicatorCode, Value)
 ...
 Population <- Population[rowSums(is.na(Population)) < ncol(Population)/2, ]
...
Population <- left_join(Popjoin, Population3, "CountryName")
 ...
 Population_score <- Population_score %>% mutate(finalrank_pop = rank(Population_score$score_pop)) %>% select("CountryName", "finalrank_pop")
 
ps <-Population_score
colnames(ps)[2] <- "Composite_Population_Rank"

ps %>% 
  arrange(Composite_Population_Rank) %>% 
  slice(1:5)
```

Sense of Security Composite score: Assessed by Strength of Legal system, and inversely related to intentional homicide rate (per 100,000)
...
security_score <- mutate(security_score, 
                     finalscore_security = as.numeric(as.factor(rank((rank.legal+rank.homi)/2))))

security_score <- security_score %>% select("CountryName", "finalscore_security")

sec_s <- security_score
colnames(sec_s)[2] <- "Composite_Security_Rank"

sec_s %>%
   arrange(Composite_Security_Rank) %>%
   slice(1:5)

```

Tourism score: Assessed by number of international departures (% of total population)
...
Population <- read.csv("https://raw.githubusercontent.com/Seranne/IntroDSproject/master/WDI%20data%20split/Population.csv")

Population <- filter(Population, Population$Year == "2013")
...
tourism_score <- tour %>% mutate(finalrank_tourism = rank(-(tour$ST.INT.DPRT/tour$SP.POP.TOTL))) %>% select("CountryName", "finalrank_tourism")

ts <-tourism_score
colnames(ts)[2] <- "Composite_Tourism_Rank"

ts %>%
   arrange(Composite_Tourism_Rank) %>%
   slice(1:5)

```

Ethnic Diversity: Ethnic Fractionalization is commonly used in Economics literature to compare the levels of ethnic, cultural, linguistic and religious fractionalization in different countries ([Wikipedia](https://en.wikipedia.org/wiki/List_of_countries_ranked_by_ethnic_and_cultural_diversity_level)). We use this as a measure of ethnic diversity in a country. Inclusion of this measure is one of the key strengths of our  project, since this was not included in earlier World Happiness Indices, but has been shown to be associated with health and [well-being](http://ftp.iza.org/dp9726.pdf). At a time when more [future research](http://www.nuffieldfoundation.org/impact-ethnic-diversity-well-being-and-health) is also being undertaken in this domain, we find it particularly relevant to include this in our index.
...
ds <- diversity
colnames(ds)[2] <- "Composite_Diversity_Rank"
ds %>%
   arrange(Composite_Diversity_Rank) %>%
   slice(1:5)
...
UN SDSN World Happiness Report Data is available for the years 2015, 2016 and 2017, the most comprehensive of which was WHR 2016. This is the report we compared against and used in our project. In particular, we extracted two variables from the 2016 World Happiness Report dataset, which we used in our composite well-being index: Citizen report of positive feelings the previous day (Positive Affect), and Citizen Feeling of negative feelings in the previous day (Negative Affect).

```{r, ECHO = FALSE}
# Load dataset: WHR 2016
url <- "https://raw.githubusercontent.com/Seranne/IntroDSproject/master/Original%20Data/online-data-chapter-2-whr-2017.csv"
WHR <- read_csv(url)
WHR <- WHR %>% select(country, `Positive affect`, `Negative affect`)

colnames(WHR)[1] <- "CountryName"
...
pos_affect <- WHR %>% 
  ...
  na.omit() %>%
  mutate(finalrank_pos = rank(mean_pa)) %>% select("CountryName", "finalrank_pos")

pos_a <- pos_affect
colnames(pos_a)[2] <- "Positive_Affect_Rank" 
pos_a %>%
   arrange(Positive_Affect_Rank) %>%
   slice(1:5)
...
neg_affect <- WHR %>% 
  ...
  na.omit() %>%
  mutate(finalrank_neg = rank(mean_na)) %>% select("CountryName", "finalrank_neg")

neg_a <- neg_affect
colnames(neg_a)[2] <- "Negative_Affect_Rank" 
neg_a %>%
   arrange(Negative_Affect_Rank) %>%
   slice(1:5)
```

**Extracting Data from the Happy Planet Index**

2016 Happy Planet Index data is available through [this link](http://happyplanetindex.org/countries). A cleaned .CSV is available through the code below, on which further analysis was run. As other variables have been Covered in previous categories, only inequality adjusted well-being indicator was selected for our score.

```{r, ECHO = FALSE}
# Load dataset
hpi_data_2016 <- read.csv("https://raw.githubusercontent.com/Seranne/IntroDSproject/master/Cleaned_Data/cleaned_hpi_2016.csv")
hpi_data_2016
colnames(hpi_data_2016)[2] <- "CountryName"
 hpi_data_2016 <- select(hpi_data_2016, "CountryName", "Inequality.adjusted_Wellbeing")
 hpi_data_2016 <- na.omit(hpi_data_2016)
 hpi_data_2016  <- hpi_data_2016 %>% distinct(CountryName, .keep_all = TRUE)
 hpi_data_2016  <- hpi_data_2016  %>% mutate(finalrank_hpi2 = rank(-hpi_data_2016 $Inequality.adjusted_Wellbeing))
 hpi_data_2016$finalrank_hpi <- as.numeric(as.factor(hpi_data_2016$finalrank_hpi2))
 
happyplanet_score <- select(hpi_data_2016,"CountryName","finalrank_hpi")

hps <-happyplanet_score
colnames(hps)[2] <- "InequalityAdjusted_WellBeing_Rank"

hps %>%
   arrange(InequalityAdjusted_WellBeing_Rank) %>%
   slice(1:5)
```
**Results**

**Deriving new well-being index, and gathering all the variable indices join to create a composite score:** 
...
final_score_table <- Communication_score %>%
  ...
  full_join(deficit_score, by='CountryName') %>%
  ...
  full_join(diversity, by='CountryName')%>%
  full_join(happyplanet_score, by='CountryName')%>%
  full_join(neg_affect, by='CountryName')%>%
  full_join(pos_affect, by='CountryName')
final_score_table
```
...
```{r, ECHO = FALSE}
# Fill in data on regions and sub-regions
...
levels(final_score$CountryName)[levels(final_score$CountryName) == "Egypt, Arab Rep."] <- "Egypt"
levels(final_score$CountryName)[levels(final_score$CountryName) == "Lao PDR"] <- "Laos"

final_score$sub.region[final_score$CountryName == "Bolivia" & is.na(final_score$sub.region)] <- "South America"

final_score$sub.region[final_score$CountryName == "Egypt" & is.na(final_score$sub.region)] <- "Northern Africa"

final_score$sub.region[final_score$CountryName == "Caribbean small states" & is.na(final_score$sub.region)] <- "South America"

final_score$sub.region[final_score$CountryName == "Venezuela" & is.na(final_score$sub.region)] <- "South America"

final_score$sub.region[final_score$CountryName == "Cote d'Ivoire" & is.na(final_score$sub.region)] <- "Western Africa"

final_score$sub.region[final_score$CountryName == "Democratic Republic of Congo" & is.na(final_score$sub.region)] <- "Middle Africa"

final_score$sub.region[final_score$CountryName == "Tanzania" & is.na(final_score$sub.region)] <- "Middle Africa"

final_score$sub.region[final_score$CountryName == "Republic of Congo" & is.na(final_score$sub.region)] <- "Western Africa"

final_score$sub.region[final_score$CountryName == "Iran" & is.na(final_score$sub.region)] <- "Western Asia"

final_score$sub.region[final_score$CountryName == "Laos" & is.na(final_score$sub.region)] <- "South-Eastern Asia"

final_score$sub.region[final_score$CountryName == "Macedonia" & is.na(final_score$sub.region)] <- "Southern Europe"

final_score$sub.region[final_score$CountryName == "Moldova" & is.na(final_score$sub.region)] <- "Eastern Europe"

final_score$sub.region[final_score$CountryName == "Russia" & is.na(final_score$sub.region)] <- "Eastern Europe"

final_score$sub.region[final_score$CountryName == "United Kingdom" & is.na(final_score$sub.region)] <- "Western Europe"

final_score$sub.region[final_score$CountryName == "South Korea" & is.na(final_score$sub.region)] <- "South-Eastern Asia"

final_score$sub.region[final_score$CountryName == "Vietnam" & is.na(final_score$sub.region)] <- "South-Eastern Asia"

final_score$sub.region[final_score$CountryName == "Syria" & is.na(final_score$sub.region)] <- "Western Asia"

final_score$region[final_score$sub.region == "South America" & is.na(final_score$region)] <- "Americas"

final_score$region[(final_score$sub.region == "Western Africa" | final_score$sub.region == "Middle Africa" | final_score$sub.region == "Northern Africa")  & is.na(final_score$region)] <- "Africa"

final_score$region[(final_score$sub.region == "Western Asia" | final_score$sub.region == "South-Eastern Asia")  & is.na(final_score$region)] <- "Asia"

final_score$region[(final_score$sub.region == "Western Europe" | final_score$sub.region == "Eastern Europe" | final_score$sub.region == "Southern Europe")  & is.na(final_score$region)] <- "Europe"

final_score <- final_score %>% select("CountryName", "region","sub.region","ourindex", "finalindex")
fscore <- final_score
colnames(fscore)[1] <- "Country"
colnames(fscore)[2] <- "Region"
colnames(fscore)[3] <- "Sub-Region"
colnames(fscore)[4] <- "Composite Score"
colnames(fscore)[5] <- "Well-Being Rank"

write.csv(fscore, file = "HHWell-Being.Index.csv")
head(fscore)
```

#Scatter plot showing the distribution of country ranks by region
After calculating the composite well-being score, we created a scatter plot of these scores. From the graph, you can see a clear clustering of ranks based on the region of the country. For example, there is a cluster of blue points towards the bottom of the graph. These implies that the best performing countries are from Europe.
```{r}
final_score %>%
  ggplot(aes(x = CountryName,
             y = finalindex,
             colour=final_score$region))+ 
...
Next, we looked at the mean of ranks per region and we can clearly see that the smaller ranks cluster in Europe.
```{r}
a<- final_score %>%  group_by(region) %>% summarise(fi = mean(finalindex))
...
final_score %>% 
  ...
  labs(colour = "Regions",
       title = "Box plot showing the distribution of country ranks by region", 
       x = "Country", 
       y = "Wellbeing Index ranking") + 
  theme_igray()
```
#Our Happiness Index
Finally, we plotted our Happy Highs Well-being score on the world map.
```{r}
par(mai=c(0,0,0.2,0),xaxs="i",yaxs="i")

#joining the data to a map
finalindexplot <- joinCountryData2Map(final_score,
                                      joinCode = "NAME",
...
#joining the data to a map
finalrank_emission <- joinCountryData2Map(final_score,
                                      joinCode = "NAME",
                                      nameJoinColumn = "CountryName"
)
...
#classify the data to a factor
finalrank_emission@data[["Happy-categories"]] <- cut(finalrank_emission@data[["finalindex"]],
...
#joining the data to a map
finalrank_genplot <- joinCountryData2Map(final_score,
                                      joinCode = "NAME",
                                      nameJoinColumn = "CountryName"
)
...
#classify the data to a factor
finalrank_genplot@data[["Happy-categories"]] <- cut(finalrank_genplot@data[["finalindex"]],
                                                 c(1,27,54,108,135,163,190),
                                                 include.lowest=TRUE )
#rename the categories
...
#joining the data to a map
finalrankdiv <- joinCountryData2Map(final_score,
                                      joinCode = "NAME",
                                      nameJoinColumn = "CountryName"
)
...
#classify the data to a factor
finalrankdiv@data[["Happy-categories"]] <- cut(finalrankdiv@data[["finalindex"]],
                                                 c(1,27,54,108,135,163,190),
                                                 include.lowest=TRUE )
#rename the categories
...
```
## Final Analysis:
From the aggregation of our individual category scores, we found that our well-being index did demonstrate a tilt towards European and American regions, with Europe producing the best ranks. However this only tells half the story, with significant shuffling of countries across the index, and the inclusion of countries such as Latvia and Estonia in the Top 10, and countries like Timor Leste and Gambia towards the bottom. Our index highlights the importance of including more relevant and encompassing variables, within population-level composite well-being indices in order to provide greater representation and acknowledgement of the factors which truly matter.

## Reasons for potentially skewed data
1)Missing data problem: Many low and lower-middle income countries lacked data resulting in over representation of upper-middle and high income countries

2)Poor data collection as reflected in the missing data

3)Small sample sizes which were not representative of the total population (eg. ~2000 records for positive and negative affects to represent the 1.25 billion strong population of countries like India)
