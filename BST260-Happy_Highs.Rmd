...
colnames(final_score)[1] <- "Country"
colnames(final_score)[2] <- "Region"
colnames(final_score)[3] <- "Sub-Region"
colnames(final_score)[4] <- "Composite Score"
colnames(final_score)[5] <- "Well-Being Rank"

write.csv(final_score, file = "HHWell-Being.Index.csv")
head(final_score)
```
...
```{r}
a<- final_score %>%  group_by(Region) %>% summarise(fi = mean("Well-Being Rank"))
...
final_score %>% 
  ...
  aes(x = fct_reorder(Region,"Well-Being Rank")) + 
  labs(colour = "Regions",
       title = "Scatter plot showing the distribution of country ranks by region", 
       x = "Country", 
